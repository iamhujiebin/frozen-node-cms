...
import crc from "crc";
import {HeartBeat, Login, GlobalGiftBanner, EnterRoom, LeaveRoom, RoomHeartBeat} from "@/proto/userProxy_pb"
...
let enterRoom = () => {
    let e = new EnterRoom();
    e.setGroupId("HTGS#a91394382")
    console.log(e)
    const binaryData = e.serializeBinary();
    hiloWebsocket.send(encodeMessage(152, serialNum, binaryData));
}
let leaveRoom = () => {
    let e = new LeaveRoom();
    e.setGroupId("HTGS#a91394382")
    console.log(e)
    const binaryData = e.serializeBinary();
    hiloWebsocket.send(encodeMessage(153, serialNum, binaryData));
}

let roomHeartbeat = () => {
    let e = new RoomHeartBeat();
    e.setGroupId("HTGS#a91394382")
    console.log(e)
    const binaryData = e.serializeBinary();
    hiloWebsocket.send(encodeMessage(154, serialNum, binaryData));
}
export {
    hiloWebsocket,
    createHiloWebSocket,
